...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                // Validate SQL scripts are syntactically correct
                sh '''
                    echo "Validating SQL scripts..."
                    for f in $(find . -name "*.sql" -type f); do
                        echo "Checking: $f"
                        # Basic syntax validation — ensure files are non-empty and parseable
                        if [ ! -s "$f" ]; then
                            echo "WARNING: Empty SQL file: $f"
                        fi
                    done
                    echo "SQL validation complete."
                '''
                // TODO: Add pg_format or sqlfluff linting if available
            }
        }

        stage('Test') {
            steps {
                // TODO: Add integration tests against a test PostgreSQL instance
                runTests(
                    testCommand: '''
    ...
    post {
        always {
            archiveArtifacts artifacts: '**/trivy-report-*.json,**/trivy-report-*.txt,**/gitleaks-report.json,**/tfsec-report*.json,**/tfsec-report*.txt', allowEmptyArchive: true
            cleanWs()
        }
        failure {
